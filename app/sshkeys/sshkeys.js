...
.controller('SSHKeysCtrl', ['KeyService', function(KeyService) {
  ...
  this.all = KeyService.getAll();
  if(this.all.keys!=null){
    this.allKeys = [];
    for(var i = 0 ; i < this.all.keys.length ; i++)
      this.allKeys.push({key:this.all.keys[i].key});
    this.sshKeys = this.allKeys;
  }
  else{
    alert('Used test keys because communication with API failed\n');
    this.sshKeys = this.testKeys;
  }
